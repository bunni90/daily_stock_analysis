...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning(t('portfolio.brokerFallbackEmpty'));
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning(t('portfolio.brokerFallbackError'));
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || t('portfolio.riskDegradedFallback'));
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(formatUiText(t('portfolio.analysisTaskSubmitted'), { symbol: row.symbol, taskId: task.taskId }));
    } catch (err) {
      setError(getParsedApiError(err));
    } finally {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning(t('portfolio.writeBlockedWarning'));
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning(t('portfolio.writeBlockedDeleteWarning'));
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning(t('portfolio.selectAccountDeleteWarning'));
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAccountCreateSuccess(t('portfolio.accountCreated'));
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || t('portfolio.accountCreateFailed'));
      setAccountCreateSuccess(null);
    } finally {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">{t('portfolio.newAccount')}</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">{t('portfolio.createAfterSwitch')}</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
              ...
              title={t('portfolio.createAccountFailedTitle')}
          ...
          {accountCreateSuccess ? (
            <InlineAlert
              variant="success"
              className="mt-2 rounded-lg px-2 py-1 text-xs shadow-none"
              title={t('portfolio.createAccountSuccessTitle')}
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={t('portfolio.baseCurrencyPlaceholder')}
            ...
            >
              <option value="cn">{t('portfolio.marketCN')}</option>
              <option value="hk">{t('portfolio.marketHK')}</option>
              <option value="us">{t('portfolio.marketUS')}</option>
              <option value="jp">{t('portfolio.marketJP')}</option>
              <option value="kr">{t('portfolio.marketKR')}</option>
              <option value="tw">{t('portfolio.marketTW')}</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? t('portfolio.creating') : t('portfolio.createAccountBtn')}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualTrade')}</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS}               placeholder={t('portfolio.symbolPlaceholder')} value={tradeForm.symbol}
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualCash')}</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <input className={PORTFOLIO_INPUT_CLASS}               placeholder={formatUiText(t('portfolio.currencyPlaceholder'), { default: writableAccount?.baseCurrency || t('portfolio.currencyPlaceholderDefault') })} value={cashForm.currency}
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualCorporate')}</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS}               placeholder={t('portfolio.symbolPlaceholderShort')} value={corpForm.symbol}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.csvImport')}</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  ...
                  <option value="huatai">{formatUiText(t('portfolio.brokerLabel'), { id: 'huatai', name: '华泰' })}</option>
                )}
              </select>
              <label className={PORTFOLIO_FILE_PICKER_CLASS}>
                {t('portfolio.selectCsv')}
            ...
            <div className="flex gap-2">
              ...
              <button type="button" className="btn-secondary flex-1"
                ...
                {csvCommitting ? t('portfolio.committing') : t('portfolio.commitImport')}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                variant={getCsvParseVariant(csvParseResult)}
                title={t('portfolio.csvParseResult')}
                message={formatUiText(t('portfolio.csvParseMessage'), { valid: csvParseResult.recordCount, skipped: csvParseResult.skippedCount, error: csvParseResult.errorCount })}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? t('portfolio.csvPreviewResult') : t('portfolio.csvCommitResult')}
                message={formatUiText(t('portfolio.csvCommitMessage'), { mode: csvDryRun ? t('portfolio.csvPreviewMode') : t('portfolio.csvWriteMode'), inserted: csvCommitResult.insertedCount, duplicate: csvCommitResult.duplicateCount, failed: csvCommitResult.failedCount })}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.eventLog')}</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">{t('portfolio.tradeLog')}</option>
                <option value="cash">{t('portfolio.cashLog')}</option>
                <option value="corporate">{t('portfolio.corporateLog')}</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? t('portfolio.refreshing') : t('portfolio.refreshLog')}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder={t('portfolio.filterSymbolPlaceholder')} value={eventSymbol}
            ...
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">{t('portfolio.allDirections')}</option>
                <option value="in">{t('portfolio.cashIn')}</option>
                <option value="out">{t('portfolio.cashOut')}</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">{t('portfolio.allActionTypes')}</option>
                <option value="cash_dividend">{t('portfolio.cashDividend')}</option>
                <option value="split_adjustment">{t('portfolio.splitAdjustment')}</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? t('portfolio.deleteHintSingle') : t('portfolio.deleteHintAll')}
            </div>
              ...
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {formatUiText(t('portfolio.tradeEventSummary'), { symbol: `${item.tradeDate} ${formatSideLabel(item.side, language)} ${item.symbol}`, quantity: item.quantity, price: item.price })}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: formatUiText(t('portfolio.confirmDeleteTrade'), { date: item.tradeDate, side: formatSideLabel(item.side, language), symbol: item.symbol, quantity: item.quantity, price: item.price }),
                      })}
                    >
                      {t('portfolio.delete')}
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: formatUiText(t('portfolio.confirmDeleteCash'), { date: item.eventDate, direction: formatCashDirectionLabel(item.direction, language), amount: item.amount, currency: item.currency }),
                      })}
                    >
                      {t('portfolio.delete')}
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: formatUiText(t('portfolio.confirmDeleteCorporate'), { date: item.effectiveDate, actionType: formatCorporateActionLabel(item.actionType, language), symbol: item.symbol }),
                      })}
                    >
                      {t('portfolio.delete')}
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title={t('portfolio.noLogTitle')}
                      description={t('portfolio.noLogDescription')}
                      className="border-none bg-transparent px-3 py-6 shadow-none"
                    />
                  ) : null}
            </div>
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>{formatUiText(t('portfolio.pageIndicator'), { current: eventPage, total: totalEventPages })}</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        message={pendingDelete?.message || t('portfolio.confirmDeleteMessage')}
