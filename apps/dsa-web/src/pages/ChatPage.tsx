...
const getMessageSkillLabel = (msg: Message, separator: string = '、'): string => getMessageSkillNames(msg).join(separator);
...
const ChatPage: React.FC = () => {
  ...
  const handleToggleWatchlist = useCallback(
    async (stockCode: string) => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setWatchlistMessage(t('chat.watchlistFailed'));
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || t('chat.compressionConfigError'));
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || t('chat.compressionSaveError'));
      } finally {
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      await startStream(payload, {
        ...
        skillName: usedSkillNames.join(t('chat.skillSeparator')),
      });
    },
    [activeStockContext, getSkillNames, input, language, loading, normalizeSelectedSkillIds, requestScrollToBottom, selectedSkillIds, sessionId, startStream],
  );
  ...
  const handleQuickQuestion = (q: (typeof QUICK_QUESTIONS)[0]) => {
    ...
    handleSend(t(q.labelKey as UiTextKey), [q.skill]);
  };
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    const skillLabel = getMessageSkillLabel(msg, t('chat.skillSeparator'));
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} ${t('chat.done')}`;
    if (last.type === 'generating')
      return last.message || t('chat.generatingFinal');
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = `${toolSteps.length} ${t('chat.toolCallsUnit')} · ${totalDuration.toFixed(1)}s`;
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          text = step.message || t('chat.thinkingStep', { step: step.step ?? 0 });
          ...
          text = step.message || t('chat.generatingAnalysis');
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={t('chat.switchToSession', { title: s.title })}
                ...
                >
                  ...
                  <div className="content">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="meta">
                        {s.message_count} {t('chat.messageCountUnit')}
                      </span>
                      {s.last_active && (
                        <>
  ...
  const selectedSkillSummary = selectedSkillIds.length > 0
    ? getSkillNames(selectedSkillIds).join(t('chat.skillSeparator'))
          ...
          <div className="flex items-start justify-between gap-4">
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              <button
                ...
                aria-label={t('chat.historySessions')}
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                ...
                <Tooltip content={t('chat.sendToNotification')}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      aria-label={t('chat.sendToNotification')}
                    >
                      {sending ? (
                        <svg
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title={t('chat.startAsk')}
                  description={t('chat.startAskDescription')}
                ...
                >
                  ...
                  >
                    {msg.role === 'assistant' && skillLabel && (
                      <div className="mb-2">
                        <Badge variant="info" className="chat-skill-badge shadow-none" aria-label={t('chat.skillBadge', { label: skillLabel })}>
                          <svg
                            className="w-3 h-3"
                            fill="none"
                            stroke="currentColor"
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          ...
                          <button
                            ...
                            className="chat-copy-btn"
                            aria-label={t('chat.exportMessageTooltip')}
                          >
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                   title={t('chat.compressionNotSaved')}
