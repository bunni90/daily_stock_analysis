...
async def auth_update_settings(request: Request, body: AuthSettingsRequest):
    ...
    if target_enabled:
        if password or confirm:
            if stored_password_exists:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": _t("auth.password_already_exists"),
                    },
                )
            if not password:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_required", "message": _t("auth.enter_password")},
                )
            if password != confirm:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_mismatch", "message": _t("auth.password_mismatch")},
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "password_required", "message": _t("auth.set_password_first")},
            )
        else:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": _t("auth.enter_current_password")},
                    )
                ...
                if not verify_stored_password(current_password):
                    ...
                    return JSONResponse(
                        status_code=401,
                        content={"error": "invalid_password", "message": _t("auth.current_password_wrong")},
                    )
                clear_rate_limit(ip)
    else:
        if current_enabled:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": _t("auth.enter_password_before_disable")},
                    )
                ip = get_client_ip(request)
                if not check_rate_limit(ip):
                    return JSONResponse(
...
async def auth_login(request: Request, body: LoginRequest):
    ...
    if not password:
        return JSONResponse(
            status_code=400,
            content={"error": "password_required", "message": _t("auth.enter_password_login")},
        )
    ...
    if not password_set:
        ...
        if not verify_password(password):
            ...
            return JSONResponse(
                status_code=401,
                content={"error": "invalid_password", "message": _t("auth.wrong_password")},
            )
...
async def auth_change_password(body: ChangePasswordRequest):
    ...
    if not current:
        return JSONResponse(
            status_code=400,
            content={"error": "current_required", "message": _t("auth.enter_current_to_change")},
        )
    if new_pwd != new_confirm:
        return JSONResponse(
            status_code=400,
            content={"error": "password_mismatch", "message": _t("auth.new_password_mismatch")},
        )
