...
class HistoryCommand(BotCommand):
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        try:
            ...
            return BotResponse.text_response(_t("bot.history.storage_unavailable"))
        ...
        if args and args[0].lower() in ("clear", "清除"):
            try:
                ...
                return BotResponse.text_response(
                    _t("bot.history.cleared", count=deleted)
                )
            except Exception as e:
                logger.error(f"History clear failed: {e}")
                return BotResponse.text_response(_t("bot.history.clear_failed", error=str(e)))
        ...
        if args and not args[0].isdigit():
            ...
            if not (session_id.startswith(prefix) or session_id == legacy_chat_session_id):
                return BotResponse.text_response(_t("bot.history.auth_check"))
            try:
                messages_list = db.get_conversation_messages(session_id, limit=20)
                if not messages_list:
                    return BotResponse.text_response(_t("bot.history.no_messages", id=session_id))

                lines = [_t("bot.history.session_detail", id=session_id), ""]
                ...
                return BotResponse.text_response(_t("bot.history.get_detail_failed", error=str(e)))
        ...
        try:
            ...
            if not sessions:
                return BotResponse.text_response(_t("bot.history.no_history"))

            lines = [_t("bot.history.header"), ""]
            for i, sess in enumerate(sessions, 1):
                title = sess.get("title", _t("bot.history.new_conversation"))
                msg_count = sess.get("message_count", 0)
                last_active = sess.get("last_active", "")[:16] if sess.get("last_active") else ""
                sid = sess["session_id"]
                lines.append(f"**{i}.** {title}")
                lines.append(f"   {_t('bot.history.msg_count', count=msg_count)} | 🕐 {last_active}")
                lines.append(f"   ID: `{sid}`")
                lines.append("")

            lines.append(_t("bot.history.footer"))
            return BotResponse.markdown_response("\n".join(lines))

        except Exception as e:
            logger.error(f"History list failed: {e}")
            return BotResponse.text_response(_t("bot.history.get_list_failed", error=str(e)))
