...
SUMMARY_USER_PREFIX_ZH = "[系统生成的历史对话摘要，仅供延续本会话]"
SUMMARY_USER_PREFIX_EN = "[System-generated conversation summary for session continuity only]"
SUMMARY_LLM_TIMEOUT_SECONDS = 20

SUMMARY_SYSTEM_PROMPT_ZH = """你是股票问答系统的会话压缩器，只能总结已经出现过的用户可见对话内容。
...
SUMMARY_SYSTEM_PROMPT_EN = """You are a conversation compressor for a stock Q&A system. Summarize only the user-visible dialogue that has already occurred.

Strict rules:
- Summarize only existing dialogue; do not add new quotes, news, financial data, or investment advice.
- Do not infer unstated facts or add new buy/sell recommendations.
- Must retain: tickers, cost basis, time horizons, risk preferences, strategy perspectives, key judgments, trade conditions, stop-loss/take-profit levels, data staleness, tool failures, and unresolved questions.
- Output must use Markdown and strictly include these 5 second-level headings:
  ## Session Summary
  ## Current Watched Tickers
  ## User Preferences & Constraints
  ## Existing Judgments & Trade Conditions
  ## Risks, Data Staleness & Unresolved Issues
"""

SUMMARY_USER_PREFIX = SUMMARY_USER_PREFIX_ZH
SUMMARY_SYSTEM_PROMPT = SUMMARY_SYSTEM_PROMPT_ZH
...
def build_summary_message(summary_text: str, *, lang: str = "en") -> Dict[str, str]:
    """Build the synthetic summary message injected into chat history."""
    prefix = SUMMARY_USER_PREFIX_EN if lang == "en" else SUMMARY_USER_PREFIX_ZH
    return {
        "role": "user",
        "content": f"{prefix}\n{summary_text.strip()}",
    }
...
def build_summary_messages(
    ...
    *,
    ...
    if previous_summary.strip():
        sections.append(("Existing rolling summary:\n" if is_en else "已有滚动摘要：\n") + previous_summary.strip())
    sections.append("New dialogue to incorporate into summary:" if is_en else "本次需要纳入摘要的新增对话：")
    sections.append(_render_visible_messages(messages))
    user_payload = "\n\n".join(sections).strip()
    system_prompt = SUMMARY_SYSTEM_PROMPT_EN if is_en else SUMMARY_SYSTEM_PROMPT_ZH
    return [
